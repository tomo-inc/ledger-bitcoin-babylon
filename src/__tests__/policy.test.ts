...
describe('stakingTxPolicy', () => {
  ...
  it("can sign a psbt", async () => {
    ...
    // 验证 tapleaf hash (修正属性名)
    //expect(partialSig0.tapleafHash).toBeDefined();
    //expect(Buffer.from(partialSig0.tapleafHash!)).toEqual(expectedLeafHash);
    
    // 验证 pubkey
    //expect(partialSig0.pubkey).toBeDefined();
    //expect(Buffer.from(partialSig0.pubkey)).toEqual(expectedPubkey);
    
    // 验证签名长度（Schnorr 签名应该是 64 字节）
    //expect(partialSig0.signature).toBeDefined();
    //expect(partialSig0.signature.length).toBeGreaterThanOrEqual(64);
